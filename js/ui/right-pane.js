...
function setupReactionDiffusionParameters() {
  ...
  // Presets dropdown
  pane
    .addInput(parameterValues, "presets", {
      label: "Presets",
  ...
  // Timestep
  pane
    .addInput(parameterValues, "timestep", {
      label: "Timestep",
...
function setupSeedFolder() {
  ...
  // Seed type dropdown
  seedFolder
    .addInput(parameterValues.seed, "type", {
      label: "Type",
      options: {
...
function addCircleOptions(folder) {
  folder.addInput(parameterValues.seed.circle, "radius", {
    ...
    min: 1,
    max:
      parameterValues.canvas.width > parameterValues.canvas.height
...
function addSquareOptions(folder) {
  ...
  folder
    .addInput(parameterValues.seed.square, "rotation", {
      label: "Rotation",
      min: -180,
      max: 180,
    })
...
//==============================================================
//  MASK (obstacle channel from seed / HELLO text in firstFrame)
//==============================================================
function setupMaskFolder() {
  const maskFolder = pane.addFolder({ title: "Mask" });

  maskFolder
    .addInput(parameterValues.mask, "enabled", {
      label: "Enable",
    })
    .on("change", (value) => {
      simulationUniforms.maskEnabled.value = value ? 1.0 : 0.0;
    });

  maskFolder
    .addInput(parameterValues.mask, "text", {
      label: "Obstacle text",
    })
    .on("change", () => {
      drawFirstFrame(currentSeedType);
  ...
  maskFolder
    .addInput(parameterValues.mask, "textSize", {
      label: "Text size",
      min: 8,
      max: 400,
      step: 1,
...
function setupRenderingFolder() {
  ...
  renderingFolder
    .addInput(parameterValues, "renderingStyle", {
      label: "Style",
      options: {
        "HSL mapping": 0,
        Gradient: 1,
  ...
  renderingFolder.addInput(parameterValues, "useSmoothing", {
    label: "Use smoothing",
  });
...
function addGradientOptions(folder) {
  ...
  folder
    ...
    .on("change", (value) => {
      displayUniforms.colorStop1.value = new THREE.Vector4(
        value.r / 255,
        value.g / 255,
        value.b / 255,
        displayUniforms.colorStop1.value.w,
      );
    });

  folder
    .addInput(parameterValues.gradientColors, "color1Stop", {
      label: "Threshold",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.colorStop1.value.w = value;
    });

  folder
    .addInput(parameterValues.gradientColors, "color1Enabled", {
      label: "Enabled",
    })
    .on("change", (checked) => {
      if (checked) {
  ...
  // Color 2 --------------------------------------------
  folder
    .addInput(parameterValues.gradientColors, "color2RGB", { label: "Color 2" })
    .on("change", (value) => {
      displayUniforms.colorStop2.value = new THREE.Vector4(
        value.r / 255,
        value.g / 255,
        value.b / 255,
        displayUniforms.colorStop2.value.w,
      );
    });

  folder
    .addInput(parameterValues.gradientColors, "color2Stop", {
      label: "Threshold",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.colorStop2.value.w = value;
    });

  folder
    .addInput(parameterValues.gradientColors, "color2Enabled", {
      label: "Enabled",
    })
    .on("change", (checked) => {
      if (checked) {
  ...
  // Color 3 --------------------------------------------
  folder
    .addInput(parameterValues.gradientColors, "color3RGB", { label: "Color 3" })
    .on("change", (value) => {
      displayUniforms.colorStop3.value = new THREE.Vector4(
        value.r / 255,
        value.g / 255,
        value.b / 255,
        displayUniforms.colorStop3.value.w,
      );
    });

  folder
    .addInput(parameterValues.gradientColors, "color3Stop", {
      label: "Threshold",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.colorStop3.value.w = value;
    });

  folder
    .addInput(parameterValues.gradientColors, "color3Enabled", {
      label: "Enabled",
    })
    .on("change", (checked) => {
      if (checked) {
  ...
  // Color 4 --------------------------------------------
  folder
    .addInput(parameterValues.gradientColors, "color4RGB", { label: "Color 4" })
    .on("change", (value) => {
      displayUniforms.colorStop4.value = new THREE.Vector4(
        value.r / 255,
        value.g / 255,
        value.b / 255,
        displayUniforms.colorStop4.value.w,
      );
    });

  folder
    .addInput(parameterValues.gradientColors, "color4Stop", {
      label: "Threshold",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.colorStop4.value.w = value;
    });

  folder
    .addInput(parameterValues.gradientColors, "color4Enabled", {
      label: "Enabled",
    })
    .on("change", (checked) => {
      if (checked) {
  ...
  // Color 5 --------------------------------------------
  folder
    .addInput(parameterValues.gradientColors, "color5RGB", { label: "Color 5" })
    .on("change", (value) => {
      displayUniforms.colorStop5.value = new THREE.Vector4(
        value.r / 255,
        value.g / 255,
        value.b / 255,
        displayUniforms.colorStop5.value.w,
      );
    });

  folder
    .addInput(parameterValues.gradientColors, "color5Stop", {
      label: "Threshold",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.colorStop5.value.w = value;
    });

  folder
    .addInput(parameterValues.gradientColors, "color5Enabled", {
      label: "Enabled",
    })
    .on("change", (checked) => {
      if (checked) {
...
function addHSLMappingOptions(folder) {
  folder
    .addInput(parameterValues.hsl.from, "min", {
      label: "Chemical range (low)",
      min: 0.0,
      max: 1.0,
      step: 0.0001,
    })
    .on("change", (value) => {
      displayUniforms.hslFrom.value.x = value;
    });

  folder
    .addInput(parameterValues.hsl.from, "max", {
      label: "Chemical range (high)",
      min: 0.0,
      max: 1.0,
      step: 0.0001,
    })
    .on("change", (value) => {
      displayUniforms.hslFrom.value.y = value;
    });

  folder
    .addInput(parameterValues.hsl.to, "min", {
      label: "Hue range (low)",
      min: 0.0,
      max: 1.0,
      step: 0.0001,
    ...
    .on("change", (value) => {
      displayUniforms.hslTo.value.x = value;
    });

  folder
    .addInput(parameterValues.hsl.to, "max", {
      label: "Hue range (high)",
      min: 0.0,
      max: 1.0,
      step: 0.0001,
    })
    .on("change", (value) => {
      displayUniforms.hslTo.value.y = value;
    });

  folder
    .addInput(parameterValues.hsl, "saturation", {
      label: "Saturation",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.hslSaturation.value = value;
    });

  folder
    .addInput(parameterValues.hsl, "luminosity", {
      label: "Luminosity",
      min: 0.0,
      max: 1.0,
    })
    .on("change", (value) => {
      displayUniforms.hslLuminosity.value = value;
    });
}
...
function setupCanvasSize() {
  ...
  if (!parameterValues.canvas.isMaximized) {
    // Width range slider
    canvasSizeFolder
      .addInput(parameterValues.canvas, "width", {
        label: "Width",
    ...
    // Height range slider
    canvasSizeFolder
      .addInput(parameterValues.canvas, "height", {
        label: "Height",
  ...
  canvasSizeFolder
    .addInput(parameterValues.canvas, "scale", {
  ...
  // Maximized checkbox
  canvasSizeFolder
    .addInput(parameterValues.canvas, "isMaximized", { label: "Maximize" })
    .on("change", (checked) => {
      if (checked) {
